...
function Shirts({ data }) {
    ...
    const allShirtProduct = data.filter((product) => product.name.includes('Shirt'))
    const tShirtProduct = allShirtProduct.filter((product) => product.name.includes('T-Shirt'))

    allShirtProduct.reduce((acc, product) => {
        if ( !tShirtProduct.includes(product)) prevProducts = [...acc, product]
        return prevProducts
    }, [])
