...
import styles from './TShirtsPolos.module.scss'
...
function TShirtsPolos({ data }) {
    const title = 'T-SHIRTS & POLO SHIRTS'

    const tShirtsProducts = data.filter((product) => product.name.includes('T-shirt'))
    const polosProducts = data.filter((product) => product.name.includes('Polo'))

    const prevProducts = [...tShirtsProducts, ...polosProducts]
