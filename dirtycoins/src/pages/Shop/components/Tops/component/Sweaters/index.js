...
import styles from './Sweaters.module.scss'
...
function Sweaters({ data }) {
    const title = 'Sweaters & Cardigans'

    const sweaterProducts = data.filter((product) => product.name.includes('Sweater'))
    const cardiganProducts = data.filter((product) => product.name.includes('Cardigan'))

    const prevProducts = [...sweaterProducts, ...cardiganProducts]
